...
class ZhiLianTransitMsg:
    ...
    def _calculate_level(self, minutes: int, status: int) -> int:
        limit = 120 if status == 5 else 1440  # 待接单2小时，待回单24小时
    ...
    def _down_transit_data(self):
        ...
        url = "https://zlzywg.chinatowercom.cn:8070/api/semp/sempWorkOrderActive/listSempWorkOrderActive"
        headers = {
            "Authorization": cookie,
        }

        data = []
        for status in (5, 6):  # 5=接单, 6=回单
            page = 1
            while True:
                body={
                    "orderStatus": status,
                ...
                try:
                    res = requests.get(url, params=body, headers=headers, verify=False)
                    result = res.json()
                    ...
                    for r in rows:
                        minutes = r["orderHandleDuration"]
                        row = {
                            "站址编码": r["stationCode"],
                            "故障状态编码": status,
                            "持续时间": minutes,
                            "发生时间": r["sendOrderTime"],
    ...
    def _process_df(self, df):
        ...
        df['故障状态'] = df['故障状态编码'].map({5: '接单', 6: '回单'})
                ...
                if res is None:
                    ...
                    if res.send_time < occur_time:
                        ...
                        if res.level < current_level:
                            ...
                            sql.commit()
                            flag = True
                   # 5级且超过2160分钟，升级到6级（二次提醒）
                    elif res.level == 5 and minutes > 2160:
                        res.level = 6
                        send_level = 6
                        res.send_time = self.now
                        sql.commit()
    ...
    def run(self):
        df = self._down_transit_data()
        df = self._process_df(df)
        self._update_send_status(df)
def main():
    ZhiLianTransitMsg().run()
